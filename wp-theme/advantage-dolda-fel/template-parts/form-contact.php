<?php
/**
 * THE contact form. One definition, used everywhere.
 *
 * Every enquiry form on the site renders from this file — the closing band on the
 * practice pages, the contact page, the 404, and anywhere the [advantage_form]
 * shortcode is dropped. Change a field here and it changes everywhere.
 *
 * Usage from a template:
 *
 *     get_template_part( 'template-parts/form', 'contact' );
 *     get_template_part( 'template-parts/form', 'contact', array( 'variant' => 'full' ) );
 *
 * Usage from the editor, Elementor, or a widget:
 *
 *     [advantage_form]
 *     [advantage_form variant="full" title="Skicka ett meddelande"]
 *
 * Args:
 *   variant  'default' (Namn, Telefon, E-post, Ämne, Meddelande)
 *            'full'    adds "Din motpart", matching the live contact page.
 *   id       Unique prefix for field ids. Auto-generated when omitted, so two
 *            forms on one page never collide.
 *   source   Free label stored with the entry, e.g. "cta-band". Helps tell apart
 *            submissions from the closing band and the contact page.
 *
...
$adf_variant = isset( $adf_args['variant'] ) ? $adf_args['variant'] : 'default';
...
<form class="form" data-contact-form novalidate
  ...
  <input type="hidden" name="adf_variant" value="<?php echo esc_attr( $adf_variant ); ?>">
